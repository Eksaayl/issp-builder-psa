...
The development gate is `npx tsc --noEmit` + `npm run lint` (+ Puppeteer smoke on port 3000); `npm run build` is deploy-only. The full operational rules live in `AGENTS.md`.
